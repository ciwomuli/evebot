...
async def _(bot: Bot, event: Event):
    ...
    try:
        name_re = await client.post(url='https://esi.evetech.net/latest/universe/ids/?datasource=tranquility&language=en', headers=headers,data=f'["{name}"]')
    except:
        await kb.finish(message=Message('当前网络连接错误，请稍后进行查询！'))
        return
    name_json = name_re.json()
    logger.info(name_json)

    if 'characters' in name_json :
        character_id = name_json['characters'][0]['id']
    else:
